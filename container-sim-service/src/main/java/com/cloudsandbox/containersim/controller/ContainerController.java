...
public class ContainerController {
    ...
    @Operation(
            summary = "Validate a Docker image against its registry",
            description = "Checks whether the given image (and tag) genuinely exists on Docker Hub before attempting to run it."
    )
    @PostMapping("/validate-image")
    public ResponseEntity<ImageValidationResult> validateImage(
            @RequestBody Map<String, String> body) {
        String image = body.get("image");
        return ResponseEntity.ok(imageValidationService.validate(image));
    }
